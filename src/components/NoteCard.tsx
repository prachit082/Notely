...
interface NoteCardProps {
  ...
  export default function NoteCard({ note, viewMode, onEdit, onDelete }: NoteCardProps) {
    return (
      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-lg hover:shadow-xl transition-all relative flex flex-col h-full">
  ...
  />
        ...
        <div className="flex items-end gap-2 mt-auto pt-4 text-black dark:text-white">
          <Button variant="secondary" onClick={() => onEdit(note.id!)} className="text-sm">
            Edit
          </Button>
          <Button variant="secondary" onClick={() => onDelete(note.id!)} className="text-sm">
            Delete
          </Button>
        </div>
